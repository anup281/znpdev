...
function login_destination(?array $user=null): string { return admin_portal_role($user)?'/admin/':'/dev/'; }
function require_login(): void {
    if(!admin_user()){$_SESSION['login_return']=$_SERVER['REQUEST_URI']??'/';header('Location: /login.php');exit;}
}
function require_admin(): void {
    require_login();
    if(!admin_portal_role()){header('Location: /dev/');exit;}
}

function investor_user(): ?array { return $_SESSION['investor_user'] ?? null; }
function require_investor_login(): void {
    if (!investor_user()) {
        $_SESSION['investor_return'] = $_SERVER['REQUEST_URI'] ?? '/portal/';
        header('Location: /login.php?tab=investor');
        exit;
    }
}
