...
if($_SERVER['REQUEST_METHOD']==='POST'){
 try{
  ...
  if(in_array($action,['archive_trade','restore_trade','delete_archived_trade'],true)){
   ...
   if($action==='delete_archived_trade'){$s=db()->prepare('DELETE FROM construction_company_trades WHERE id=? AND construction_company_id=? AND archived_at IS NOT NULL');$s->execute([$linkId,$companyId]);if($s->rowCount()<1)throw new RuntimeException('Archived vendor trade could not be found.');header('Location: companies.php?deleted=1');exit;}
...
<?php if($archivedTradeRows):?><section class="card archived-vendors"><h2>Archived Vendor Trades</h2><?php foreach($archivedTradeRows as $vendor):?><div class="archived-vendor-row"><span><strong><?=e($vendor['company_name'].' — '.$vendor['trade_name'])?></strong><small>Archived <?=e(dev_datetime($vendor['archived_at']))?></small></span><form method="post"><input type="hidden" name="csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="restore_trade"><input type="hidden" name="company_id" value="<?=(int)$vendor['company_id']?>"><input type="hidden" name="link_id" value="<?=(int)$vendor['link_id']?>"><div class="actions no-top"><button class="btn btn-secondary">Restore Trade</button><button class="btn btn-danger" name="action" value="delete_archived_trade" onclick="return confirm('Permanently delete this archived vendor trade?') && confirm('This cannot be undone. Are you absolutely sure?');">Delete</button></div></form></div><?php endforeach;?></section><?php endif;?>
