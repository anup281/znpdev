...
$ids=array_map('intval',array_column($projects,'id'));$buildingCounts=[];$taskCounts=[];$inspectionCounts=[];
if($ids){
 $in=implode(',',$ids);
 ...
 try {foreach(db()->query("SELECT construction_project_id,COUNT(*) c FROM construction_tasks WHERE construction_project_id IN ($in) AND status<>'Complete' GROUP BY construction_project_id")->fetchAll() as $r)$taskCounts[(int)$r['construction_project_id']]=(int)$r['c'];} catch(Throwable $e){error_log('Construction project task counts: '.$e->getMessage());}
