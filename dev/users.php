...
if($_SERVER['REQUEST_METHOD']==='POST'&&csrf_check((string)($_POST['csrf']??''))){
 try{
  ...
  if($action==='create_user'){
    $name=trim((string)($_POST['full_name']??''));$username=strtolower(trim((string)($_POST['username']??'')));$email=strtolower(trim((string)($_POST['email']??'')));$password=(string)($_POST['password']??'');$role=(string)($_POST['construction_role']??'Field Staff / Helper');
    if($name===''||!preg_match('/^[a-z0-9._]{3,40}$/',$username)||!filter_var($email,FILTER_VALIDATE_EMAIL)||strlen($password)<8)throw new RuntimeException('Name, valid email, and a temporary password of at least 8 characters are required.');
    $exists=db()->prepare('SELECT id FROM admin_users WHERE email=? OR LOWER(username)=? LIMIT 1');$exists->execute([$email,$username]);if($exists->fetchColumn())throw new RuntimeException('A user with that email already exists.');
    $pc=dev_admin_password_column_v3();if(!$pc)throw new RuntimeException('Could not identify the password column in admin_users.');
    $sql="INSERT INTO admin_users(full_name,username,email,`$pc`,role,is_active,construction_only,construction_role,must_change_password) VALUES(?,?,?,?,?,1,1,?,1)";
    db()->prepare($sql)->execute([$name,$username,$email,password_hash($password,PASSWORD_DEFAULT),'construction_user',$role]);
