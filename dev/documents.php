...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if($action==='delete'){
            if(!dev_is_super())throw new RuntimeException('Only an Admin or Super Admin may delete project documents.');
            ...
            $q=db()->prepare('SELECT id,document_name FROM construction_documents WHERE id=? AND construction_project_id=? AND archived_at IS NULL');
            ...
            db()->prepare('UPDATE construction_documents SET archived_at=NOW() WHERE id=? AND construction_project_id=? AND archived_at IS NULL')->execute([$documentId,$projectId]);
