...
try{
    ...
    if(!manage_receipt_payments_schema_ready())throw new RuntimeException('Payment records are not installed.');
    $query=db()->prepare('SELECT id FROM management_receipt_periods WHERE management_property_id=? AND report_year=? AND report_month=?');$query->execute([$propertyId,$year,$month]);$periodId=(int)$query->fetchColumn();if($periodId<1)throw new RuntimeException('The selected receipt period is unavailable.');
    ...
    $categoryTotals=[];
    $query=db()->prepare("SELECT COALESCE(c.category_name,'UNCATEGORIZED') category_name,SUM(t.amount) total,COUNT(*) item_count,SUM(CASE WHEN t.category_id IS NULL THEN 1 ELSE 0 END) uncategorized_count FROM management_receipt_transactions t JOIN management_receipt_periods p ON p.id=t.management_receipt_period_id LEFT JOIN management_receipt_categories c ON c.id=t.category_id LEFT JOIN management_receipt_analysis_runs r ON r.id=t.analysis_run_id WHERE p.management_property_id=? AND p.report_year=? AND p.report_month=? AND (r.is_current=1 OR t.analysis_run_id IS NULL) GROUP BY COALESCE(c.category_name,'UNCATEGORIZED')");
    $query->execute([$propertyId,$year,$month]);$uncategorizedCount=0;$receiptCount=0;foreach($query->fetchAll() as $row){$categoryName=(string)$row['category_name'];$categoryTotals[$categoryName]=(float)$row['total'];$uncategorizedCount+=(int)$row['uncategorized_count'];$receiptCount+=(int)$row['item_count'];}if($receiptCount<1)throw new RuntimeException('Upload and analyze a statement before sending to the CPA.');ksort($categoryTotals,SORT_NATURAL|SORT_FLAG_CASE);if($uncategorizedCount>0)throw new RuntimeException('All receipts must be categorized before sending to the CPA. '.$uncategorizedCount.' receipt'.($uncategorizedCount===1?' remains':'s remain').' uncategorized.');

    $query=db()->prepare('SELECT payment_date,bank_account_paid_from,amount_paid FROM management_receipt_payments WHERE management_receipt_period_id=? ORDER BY payment_date,id');$query->execute([$periodId]);$payments=$query->fetchAll();
    $categoryTotal=array_sum($categoryTotals);$paymentTotal=array_sum(array_map(static fn(array $payment):float=>(float)$payment['amount_paid'],$payments));$remainingBalance=$categoryTotal-$paymentTotal;if(abs($remainingBalance)>=0.005)throw new RuntimeException('The remaining balance must be $0.00 before sending to the CPA. Current remaining balance: '.(($remainingBalance<0)?'-$':'$').number_format(abs($remainingBalance),2).'.');
    ...
    $html.='</tbody><tfoot><tr><th style="'.$cellStyle.'" colspan="2">TOTAL PAYMENTS</th><th style="'.$cellStyle.'">'.$money($paymentTotal).'</th></tr></tfoot></table>';
    $html.='<table style="'.$tableStyle.'"><tbody><tr><th style="'.$cellStyle.'">STATEMENT NET TOTAL</th><td style="'.$cellStyle.'">'.$money($categoryTotal).'</td></tr><tr><th style="'.$cellStyle.'">TOTAL PAYMENTS</th><td style="'.$cellStyle.'">'.$money($paymentTotal).'</td></tr><tr><th style="'.$cellStyle.'">REMAINING BALANCE</th><td style="'.$cellStyle.'">'.$money($remainingBalance).'</td></tr></tbody></table>';

    db()->beginTransaction();db()->prepare("UPDATE management_receipt_periods SET status='finalized',updated_by_admin_user_id=? WHERE id=?")->execute([(int)($managementUser['id']??0)?:null,$periodId]);db()->prepare('INSERT INTO management_cpa_deliveries(management_property_id,report_year,report_month,recipient_email,cc_email,recipient_name,sent_by_admin_user_id,category_total,payment_total,sent_at) VALUES(?,?,?,?,?,?,?,?,?,NOW())')->execute([$propertyId,$year,$month,$cpaEmail,$ccEmail,trim(setting('management_cpa_name'))?:null,(int)($managementUser['id']??0)?:null,$categoryTotal,$paymentTotal]);
    $result=app_send_mail_detailed($cpaEmail,'Receipts Breakdown - '.$property['property_name'].' - '.$monthName.' '.$year,$html,[],[$ccEmail]);
    if(!$result['ok'])throw new RuntimeException('The CPA email could not be sent: '.$result['error']);db()->commit();
    manage_cpa_redirect($redirect,'success','The breakdown and payment history were emailed to '.$cpaEmail.', copied to '.$ccEmail.', and the receipt month was finalized.');
}catch(Throwable $error){if(db()->inTransaction())db()->rollBack();error_log('CPA receipt email failed: '.$error->getMessage());manage_cpa_redirect($redirect,'error',$error->getMessage());}
