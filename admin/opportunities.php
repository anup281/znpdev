...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!csrf_check($_POST['csrf_token'] ?? '')) $error = 'Your session expired. Refresh and try again.';
        ...
        if ($action === 'delete') {
            ...
            $linked = 0;
            foreach ($checks as [$table,$column]) { try { $q=db()->prepare("SELECT COUNT(*) FROM $table WHERE $column=?"); $q->execute([$opportunityId]); $linked += (int)$q->fetchColumn(); } catch (Throwable $e) { error_log('Investment link check failed for '.$table.'.'.$column.': '.$e->getMessage()); $error='The investment dependencies could not be verified, so it was not deleted.'; } }
