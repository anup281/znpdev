...
$archivedInvestment=(string)($opportunity['status']??'')==='archived'||((string)($opportunity['status']??'')===''&&(int)($opportunity['is_visible']??0)===0&&(int)($opportunity['accepting_inquiries']??0)===0);
$readOnlyInvestment=$investmentViewer||$archivedInvestment;
$requestMethod=(string)($_SERVER['REQUEST_METHOD']??'GET');if($readOnlyInvestment&&$requestMethod!=='GET'){http_response_code(403);exit($archivedInvestment?'Archived investments are view only.':'This account has read-only investment access.');}
...
?>
<div class="admin-page-head"><div><h1><?=e($opportunity['project_name'])?></h1><p><?=e(trim(($opportunity['city']??'').', '.($opportunity['state']??''),', '))?> · <?=e($statusLabels[$opportunity['status']]??ucwords(str_replace('_',' ',(string)$opportunity['status'])))?></p></div><div class="admin-row-actions"><a class="secondary" href="opportunities.php">Back to Investments</a><?php if(!$readOnlyInvestment):?><a class="secondary" href="opportunity_edit.php?id=<?=$id?>">Edit Project</a><?php endif;?></div></div>
...
<?php if($archivedInvestment):?><div class="status info">This investment is archived. All information is view only until an administrator restores it.</div><?php endif;?>
<?php if($tab==='overview'):?>
...
<?php if(!$readOnlyInvestment):?><div class="admin-document-toolbar admin-document-toolbar-above"><a class="primary admin-new-template-button" href="?id=<?=$id?>&tab=documents&new=1">+ New File</a></div><?php endif;?>
  ...
  <section class="admin-model-settings-card" id="settings-<?=e($groupId)?>" <?=$modelSettingsReady?'':'inert'?>>
   ...
   <div class="admin-model-settings-fields">
    ...
    <label class="admin-model-setting">
     <span><?=e($label)?></span>
     <span class="admin-model-input-wrap"><?php if($suffix==='$'):?><b>$</b><?php endif;?><input data-autosave type="number" min="0" <?=$whole?'step="1"':'step="0.01"'?> name="<?=e($name)?>" value="<?=e(isset($modelSettings[$name])?(string)$modelSettings[$name]:'')?>" autocomplete="off" <?=($modelSettingsReady&&!$readOnlyInvestment)?'':'disabled'?>><?php if($suffix!=='$'):?><em><?=e($suffix)?></em><?php endif;?></span>
...
<section class="admin-investor-structure" data-investor-structure data-project-id="<?=$id?>" data-csrf="<?=e(csrf_token())?>" <?=$readOnlyInvestment?'data-read-only="1"':''?>>
 <header class="admin-investor-hero">
  <div><span>Project Capitalization</span><h2>Investors &amp; Structure</h2><p><?=$readOnlyInvestment?'View':'Manage'?> the GP ownership structure and LP unit subscriptions for <?=e($opportunity['project_name'])?>.</p></div>
 </header>
 ...
 <div class="admin-investor-columns">
  <section class="admin-investor-panel">
   <header><div><span>General Partner</span><h3>GP Ownership</h3><p>Ownership percentages across all GP members must total no more than 100%.</p></div><?php if(!$readOnlyInvestment):?><button class="primary admin-small-button" type="button" data-add-investor="GP">+ Add GP</button><?php endif;?></header>
   <progress class="admin-investor-progress" data-gp-progress value="0" max="100" aria-label="GP ownership allocated">0%</progress>
   <div class="admin-investor-list" data-gp-list><div class="admin-investor-empty">Loading GP structure…</div></div>
  </section>
  <section class="admin-investor-panel">
   <header><div><span>Limited Partners</span><h3>LP Unit Subscriptions</h3><p>One LP unit represents one dollar of subscribed investment.</p></div><?php if(!$readOnlyInvestment):?><button class="primary admin-small-button" type="button" data-add-investor="LP">+ Add LP</button><?php endif;?></header>
   <label class="admin-investor-unit-setting"><span>Total LP Units Available</span><span><b>$</b><input type="number" min="0.01" step="0.01" data-lp-total-units autocomplete="off" <?=$readOnlyInvestment?'disabled':''?>><?php if(!$readOnlyInvestment):?><button class="secondary admin-small-button" type="button" data-save-units>Save</button><?php endif;?></span><small data-unit-status></small></label>
...
<?php if($tab==='model-settings'&&!$readOnlyInvestment):?><script src="../assets/admin-model-settings.js?v=20260723-536"></script><?php endif;?>
<?php if($tab==='investors'):?><script src="../assets/admin-investors.js?v=20260729-601"></script><?php endif;?>
