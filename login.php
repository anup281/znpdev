...
if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if($ok){
            ...
            $_SESSION['admin_user']=portal_session_user($u);
        ...
        if($error==='')$error=$locked?'This account is temporarily locked after multiple unsuccessful attempts. Please try again in 15 minutes or use Forgot Password.':'Invalid email address or password.';
}
require __DIR__.'/includes/header.php';
?>
<main class="public-login-shell"><section class="public-login-card">
<form method="post" class="portal-login-form"><h1>Portal Login</h1><p>Access the ZNP Development administrative and construction portals.</p><?php if($message):?><div class="status success"><?=e($message)?></div><?php endif;?><?php if($error):?><div class="status error"><?=e($error)?></div><?php endif;?><label>Email Address<input type="email" name="email" autocomplete="email" required autofocus></label><label>Password<input type="password" name="password" autocomplete="current-password" required></label><button class="primary" type="submit">Login</button><a class="admin-forgot-link" href="admin/forgot_password.php">Forgot Password?</a></form>
</section></main>
